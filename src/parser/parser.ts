...
function parse(source: string, fileName: string): Root {
...
function parseRawAST(raw: RawChildNode[], fileName: string): Node[] {
    const nodes: Node[] = [];
    ...
    for (const node of raw) {
        switch (node.nodeName) {
            ...
            case "logic":
                ...
                if (logicNode) nodes.push(logicNode);
                break;
            default:
