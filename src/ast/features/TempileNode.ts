...
class TempileNode extends Node {
    ...
    public static newFromRawNode(node: RawChildNode, fileName: string, rawAstParserFn: RawASTParserFunction): Node {
        ...
        if (node.tagName !== "tempile") {
            throw new Error(
                `Expected element tag name 'tempile' but found '${node.tagName}'. ` +
                `File: ${fileName}, Line: ${node.sourceCodeLocation?.startLine ?? "?"}, Col: ${node.sourceCodeLocation?.startCol ?? "?"}`
            );
        }
        ...
        if (atAttrs.length < 1) {
            throw new Error(
                `Missing @[type] attribute on <tempile> tag. ` +
                `File: ${fileName}, Line: ${node.sourceCodeLocation?.startLine ?? "?"}, Col: ${node.sourceCodeLocation?.startCol ?? "?"}`
            );
        } else if (atAttrs.length > 1) {
            throw new Error(
                `Multiple @ attributes on <tempile> tag are not allowed: found [${atAttrs.map(a => a.name).join(", ")}]. ` +
                `File: ${fileName}, Line: ${node.sourceCodeLocation?.startLine ?? "?"}, Col: ${node.sourceCodeLocation?.startCol ?? "?"}`
            );
        }
        ...
        if (!allowedAttrs.includes(nodeTypeAttr)) {
            throw new Error(
                `Invalid @ attribute '${nodeTypeAttr}' on <tempile> tag. Expected one of: ${allowedAttrs.join(", ")}. ` +
                `File: ${fileName}, Line: ${node.sourceCodeLocation?.startLine ?? "?"}, Col: ${node.sourceCodeLocation?.startCol ?? "?"}`
            );
        }
